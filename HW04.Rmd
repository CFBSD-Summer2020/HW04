...
## 2. Generate a data frame that summarizes the number of gun deaths per month and print with kable()
...
kable(deaths_per_month_df, col.names = c("Month", "Number of gun deaths"), align = "lc", format = "markdown")
...
```{r police_3}
race_age_police_perc <- by_race_age %>%
    mutate(Percentage = Number/sum(Number))

race_age_police_perc


race_age_police_perc_plot <- ggplot(race_age_police_perc, aes(x = Race, y = Percentage, fill = Police)) +
  geom_bar(position = "fill", stat="identity", color = "black") +
  scale_y_continuous(breaks = c(0, 0.25, 0.5, 0.75, 1.00), labels = scales::percent) +
