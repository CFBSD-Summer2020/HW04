...
#To a more understandable variables
...
#Names are too large for my taste
...
**Answer**
About police involvement and race, it seems that police officers as not as involved with the total number of deaths per race. In general, numbers are at least a order of magnitude less when police is involved, that's why a log 10 scale was necessary to observe all values clearly inside the graph. And also why I included the table.
...
**Answer**
The distribution of victim's age is not exactly normal, see the Kolmogorov-Smirnov test results (p < 0.001), also noticeable in the violin plot, so the Mann-Whitman-Wilcoxon test is a better option to assess statistical differences between groups. Although I didn't test for heterocedasticity. Police involvent in significantly different when comparing age victims
