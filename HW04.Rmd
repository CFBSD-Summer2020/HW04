...
kable(gun_deaths, format = "markdown")
```
...
gun_deaths_by_month <- gun_deaths %>%
    ...
    count(year, name = "deaths") %>% #Enumerates gun deaths
    mutate(abMonth = month.abb[month], sd = sd(deaths)) %>% #Adds column of abbreviated month name and standard error
    ungroup()

#Makes abbreviated months leveled factors
gun_deaths_by_month$abMonth <- factor(gun_deaths_by_month$abMonth, 
                                      levels = c(month.abb[12:1]))
#Plots data
set.seed(3)
gun_deaths_by_month  %>% 
    ggplot(mapping = aes(x = abMonth, y = deaths)) +
    geom_bar(stat = "summary", fun = "mean", alpha = 0.6, width = 0.4) +
    geom_errorbar(stat = "summary", mapping = aes(ymin = deaths - sd, 
                                                  ymax = deaths + sd, width = 0.2)) +
    scale_y_continuous() + coord_flip() +
    xlab("Month") + ylab("American Gun Deaths\n(average, standard deviation)") +
    ggtitle("American Gun Deaths (2012-2014)")
```
