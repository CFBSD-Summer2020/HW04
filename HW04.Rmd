...
ggplot(intent, aes(x = reorder(intent, -n) , y = n)) + geom_col()
...
ggplot(guns, aes(x= sex,  y = age)) + geom_boxplot()
...
ggplot(white_intent, aes(x=reorder(intent, -n), y=n)) + geom_col()
...
ggplot(black_intent, aes(x=reorder(intent, -n), y=n)) + geom_col()

hispanic_race <- guns %>% filter(race == "Hispanic")
hispanic_intent <- hispanic_race %>% count(intent)
ggplot(hispanic_intent, aes(x=reorder(intent, -n), y=n)) + geom_col()
